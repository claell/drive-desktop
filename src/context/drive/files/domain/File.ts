...
export class File extends AggregateRoot {
  ...
  update(current: File, trackerId: string) {
    const { path: previousPath } = this.attributes();

    if (current.name !== this.name) {
      this.record(
        new FileRenamedDomainEvent({
          aggregateId: current.contentsId,
        })
      );
    }

    if (current.folderId !== this.folderId) {
      this.record(
        new FileMovedDomainEvent({
          aggregateId: current.contentsId,
          trackerId,
    ...
    this._folderId = current.folderId;
    this._path = new FilePath(current.path);
    this._size = new FileSize(current.size);
